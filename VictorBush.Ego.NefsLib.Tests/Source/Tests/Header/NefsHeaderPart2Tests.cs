...
namespace VictorBush.Ego.NefsLib.Tests.Header
{
    ...
    public class NefsHeaderPart2Tests
    {
        ...
        public void NefsHeaderPart2_MultipleItems_EntriesPopulated()
        {
            ...
            var file1 = new NefsItem(new NefsItemId(0), "file1", new NefsItemId(0), file1DataSource, Part6Flags.None);
            items.Add(file1);

            var file2DataSource = new NefsItemListDataSource(items, 456, new NefsItemSize(789, new List<UInt32> { 14, 15, 16 }));
            var file2 = new NefsItem(new NefsItemId(1), "file2", new NefsItemId(1), file2DataSource, Part6Flags.None);
            items.Add(file2);

            var dir1DataSource = new NefsEmptyDataSource();
            var dir1 = new NefsItem(new NefsItemId(2), "dir1", new NefsItemId(2), dir1DataSource, Part6Flags.IsDirectory);
            items.Add(dir1);

            var file3DataSource = new NefsItemListDataSource(items, 222, new NefsItemSize(333, new List<UInt32> { 22, 23, 24 }));
            var file3 = new NefsItem(new NefsItemId(3), "file3", dir1.Id, file3DataSource, Part6Flags.None);
            items.Add(file3);
